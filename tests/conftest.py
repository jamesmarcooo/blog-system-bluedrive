...
def user_factory() -> Callable[..., User]:
    def create_user(username: str, is_staff: bool=False) -> User: # noqa: FBT001, FBT002
...
@pytest.fixture()
def author_factory(user_factory: Callable) -> Callable[..., Author]:
...
@pytest.fixture()
def post_factory(author_factory: Callable) -> Callable[..., Post]:
    def create_post(
        author: Author=None,
...
@pytest.fixture()
def multiple_posts(post_factory: Callable, author_factory:Callable) -> List[Post]:
    ...
    return [
        ...
        post_factory(
            author=author1,
            title="Active Post by Author One",
            active=True,
            status=PostStatus.PUBLISHED,
        ),
        post_factory(
            author=author2,
            title="Active Post by Author Two",
            active=True,
            status=PostStatus.PUBLISHED,
        ),
        # Inactive Post
...
def authenticated_author_client(
    api_client: Callable[[], APIClient],
    author_factory: Callable[..., Author],
) -> tuple[APIClient, Author]:
